...
public:
    ...
    public:
        ...
        //! @param T 取得するシーンクラス
        //! @return シーン
        template<class T> static std::shared_ptr<T> GetScene();

        //! シーンを消去します
        //! @param name シーン名
        template<class T> static void ReleaseScene();
    ...
    template<class T> static void ReleaseObject()
    {
    ...
    //! @return オブジェクト
    template<class T> static std::vector<std::shared_ptr<T>> GetObjectsPtr()
    {
...
//! @return 複数の指定オブジェクト
template<class T> std::vector<std::shared_ptr<T>> Scene::Base::GetObjectsPtr()
{
