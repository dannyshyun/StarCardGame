...
void ComponentModel::GUI()
{
        ...
        if( ImGui::TreeNode( "Model" ) )
        {
            ...
            if( ImGui::TreeNode( u8"モデル姿勢" ) )
            {
                ImGui::DragFloat4( u8"Ｘ軸",
                                   model_transform_.f32_128_0,
                                   0.01f,
                                   -10000.0f,
                                   10000.0f,
                                   "%.2f" );
                ImGui::DragFloat4( u8"Ｙ軸",
                                   model_transform_.f32_128_1,
                                   0.01f,
                                   -10000.0f,
                                   10000.0f,
                                   "%.2f" );
                ImGui::DragFloat4( u8"Ｚ軸",
                                   model_transform_.f32_128_2,
                                   0.01f,
                                   -10000.0f,
                                   10000.0f,
                                   "%.2f" );
                ImGui::DragFloat4( u8"座標",
                                   model_transform_.f32_128_3,
                                   0.01f,
                                   -10000.0f,
                                   10000.0f,
                                   "%.2f" );
    ...
    if( node_manipulate_ )
    {
        matrix matx = MV1GetFrameLocalWorldMatrix( GetModel(),
                                                   select_node_index_ );
        auto   trns = GetOwner()->GetComponent<ComponentTransform>();
...
ComponentModelPtr
    ComponentModel::SetAnimation( const std::vector<Animation::Desc> anims )
{
    if( model_ )
    {
        // エディター用アニメーション設定用
...
void ComponentModel::PlayAnimationNoSame( std::string_view name,
                                          bool             loop,
                                          float            blend_time,
                                          float            start_time )
{
...
void ComponentModel::PlayAnimation( std::string_view name,
                                    bool             loop,
                                    float            blend_time,
                                    float            start_time )
{
