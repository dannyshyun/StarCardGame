...
void ComponentCamera::SetCameraTransform()
{
    ...
    if( ! ImGuizmo::IsUsing() )
    {
        ...
        mat_proj_ = matrix::perspectiveFovLH( fovy_ * DegToRad,
                                              aspect_ratio_,
                                              near_z_,
                                              far_z_ );
    }
...
ComponentCamera::CameraRay ComponentCamera::MousePositionRay( int mouse_pos_x,
                                                              int mouse_pos_y )
{
    VECTOR start = ConvScreenPosToWorldPos(
        { (float)mouse_pos_x, (float)mouse_pos_y, 0.0f } );
    VECTOR end = ConvScreenPosToWorldPos(
        { (float)mouse_pos_x, (float)mouse_pos_y, 1.0f } );
    ...
    ImGui::Begin( obj_name.data() );
    {
        ...
        if( ImGui::TreeNode( u8"Camera" ) )
        {
            ...
            if( ImGui::TreeNode( u8"カメラステータス" ) )
            {
                ...
                bool req = ImGui::CheckboxFlags( u8"カレントカメラ",
                                                 &val,
                                                 1 << (int)CameraBit::Current );
                ImGui::CheckboxFlags( u8"タイムライン使用",
                                      bit,
                                      1 << (int)CameraBit::EnableTimeLine );
                ImGui::CheckboxFlags( u8"カメラ【視錐台(Frustum)】を表示",
                                      bit,
            ...
            ImGui::DragFloat3( u8"カメラ座標",
                               &position_.f32[0],
                               0.1f,
                               -10000.0f,
                               10000.0f,
                               "%.2f" );
            ImGui::DragFloat3( u8"注視点",
                               &look_at_.f32[0],
                               0.1f,
                               -10000.0f,
                               10000.0f,
                               "%.2f" );
            ImGui::DragFloat3( u8"上ベクトル",
                               &up_.f32[0],
                               0.1f,
                               -10000.0f,
                               10000.0f,
            ...
            bool nf = false;
            nf |= ImGui::DragFloat( u8"Near",
                                    &near_z_,
                                    0.01f,
                                    -10000.0f,
                                    10000.0f,
                                    "%.2f" );
            nf |= ImGui::DragFloat( u8"Far",
                                    &far_z_,
                                    0.01f,
                                    -10000.0f,
                                    10000.0f,
