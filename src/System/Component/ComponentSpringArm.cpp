...
void ComponentSpringArm::LateUpdate()
{
    ...
    if( auto object = object_.lock() )
    {
        ...
        matrix mat = HelperLib::Math::LookAtMatrixForObject( my_pos,
                                                             target_pos );

        float3 ofs = mul( float4( spring_arm_offset_, 0 ), mat ).xyz;
        mat        = HelperLib::Math::LookAtMatrixForObject( my_pos,
                                                      target_pos + ofs );
...
{
    float  trans[3] = { 0, 0, 0 };
    float  rot[3]   = { spring_arm_rotate_.x,
                        spring_arm_rotate_.y,
                        spring_arm_rotate_.z };
    ...
    ImGui::Begin( obj_name.data() );
    {
        ...
        if( ImGui::TreeNode( u8"SpringArm" ) )
        {
            ...
            ImGui::CheckboxFlags( u8"初期化済",
                                  &val,
                                  1 << (int)SpringArmBit::Initialized );
            ...
            ImGui::DragFloat3( u8"SpringArm回転",
                               (float*)&spring_arm_rotate_,
                               0.1f,
                               -10000.0f,
                               10000.0f,
                               "%.1f" );
            ImGui::DragFloat3( u8"SpringArmオフセット",
                               (float*)&spring_arm_offset_,
                               0.1f,
                               -10000.0f,
                               10000.0f,
                               "%.1f" );
            ImGui::DragFloat( u8"SpringArm長さ",
                              (float*)&spring_arm_length_,
                              0.1f,
                              -0.1f,
                              1000.0f,
                              "%.1f" );

            ImGui::DragFloat( u8"SpringArm 固さ",
                              (float*)&spring_arm_strong_,
                              0.1f,
                              0.0f,
                              1.0f );
            ImGui::DragFloat( u8"SpringArm 戻り",
                              (float*)&spring_arm_return_,
                              0.1f,
                              0.0f,
                              1.0f );
