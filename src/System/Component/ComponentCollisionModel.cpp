...
void ComponentCollisionModel::Draw()
{
    ...
    {
        float3 p0 = cast(
            ref_poly_.Vertexs[ref_poly_.Polygons[i].VIndex[0]].Position );
        float3 p1 = cast(
            ref_poly_.Vertexs[ref_poly_.Polygons[i].VIndex[1]].Position );
        float3 p2 = cast(
            ref_poly_.Vertexs[ref_poly_.Polygons[i].VIndex[2]].Position );
...
void ComponentCollisionModel::GUI()
{
    ...
    {
        ImGui::Separator();
        auto ui_name = std::string( "Collision Model:" ) +
...
void ComponentCollisionModel::AttachToModel( bool update )
{
    if( auto mdl = GetOwner()->GetComponent<ComponentModel>() )
    {
        ...
        assert(
            "マップのスケールのXYZが異なります。[無視する]で平均をとります." &&
            same );
...
//! @param col 相手のコリジョン
//! @return HitInfoを返す
ComponentCollisionModel::HitInfo
    ComponentCollisionModel::IsHit( ComponentCollisionPtr col )
{
    HitInfo info;
    ...
    switch( col->GetCollisionType() )
    {
        case ComponentCollision::CollisionType::LINE :
            return isHit( std::dynamic_pointer_cast<ComponentCollisionModel>(
                              shared_from_this() ),
                          std::dynamic_pointer_cast<ComponentCollisionLine>(
                              col ) );
            break;
...
const matrix ComponentCollisionModel::GetWorldMatrix() const
{
    ...
    auto obj = GetOwner();
    {
        ...
        if( mdl )
        {
            transform = mul( transform, mdl->GetMatrix() );
        }
        auto cmp = obj->GetComponent<ComponentTransform>();
        if( cmp )
        {
            transform = mul( transform, cmp->GetMatrix() );
        }
    }
...
//! @return 実際動ける量
float3 ComponentCollisionModel::checkMovement( float3 pos,
                                               float3 vec,
                                               float  force )
{
    ...
    {
        hit_poly =
            MV1CollCheck_Line( mdl->GetModel(), -1, cast( top ), cast( btm ) );
        if( hit_poly.HitFlag )
        {
