...
namespace
{
    ...
    f32 delta_time_ =
        0.0f;  //!< 1フレームの経過時間（CPUとGPU, ScreenFlip()更新待ちすべて含む）
...
void ShowFps( f32 delta )
{
    ...
    s32 refresh_rate = 60;
    {
        ...
        refresh_rate = GetDeviceCaps( hdc,
                                      VREFRESH );  // リフレッシュレートの取得
    ...
    if( ImGui::Begin( u8"FPS計測", nullptr, window_flags ) )
    {
        ...
        fps_data.AddPoint( t,
                           frame_rate / static_cast<f32>(
                                            refresh_rate ) );  // フレームレート

        static float history = 10.0f;

        constexpr ImPlotFlags flags = ImPlotFlags_NoInputs |
                                      ImPlotFlags_NoFrame;
    ...
    //----------------------------------------------------------
    ImGui::Separator();
    ImGui::Text( u8"FPS    : %3.2f fps (max:%3d fps)",
                 frame_rate,
                 refresh_rate );
...
void SystemInit()
{
    ...
    // 描画先テクスチャを作成
    //----------------------------------------------------------
    texture_hdr_ = std::make_shared<Texture>( WINDOW_W,
                                              WINDOW_H,
                                              DXGI_FORMAT_R16G16B16A16_FLOAT );

    // シェーダー読込
    shader_ps_tonemapping_ = std::make_shared<ShaderPs>(
        "data/Shader/ps_tonemapping" );
}
        ...
        if( ImGui::BeginMainMenuBar() )
        {
            ...
            if( ImGui::BeginMenu( "Scene" ) )
            {
                ...
                if( ImGui::BeginMenu( u8"シーン選択" ) )
                {
                    ...
                    {
                        // 表示文字列
                        std::string scene_name = std::string( p->className() ) +
                                                 " - " + p->descName();
            ...
            if( ImGui::BeginMenu( "Debug" ) )
            {
                ...
                if( debug_camera )
                {
                    ...
                    {
                        ImGui::RadioButton( "Unity",
                                            (int*)( &control ),
                                            DebugCamera::Unity );
                        ImGui::RadioButton( "UnrealEngine",
                                            (int*)( &control ),
                                            DebugCamera::UnrealEngine );
                        ImGui::RadioButton( "Maya",
                                            (int*)( &control ),
                                            DebugCamera::Maya );
                        ImGui::EndMenu();
                    }
                        ...
                        Scene::EditorStatusBit::EditorPlacement ) )
                {
                    ImGui::CheckboxFlags( u8"Always",
                                          (int*)&Scene::SceneStatus(),
                                          1 << (int)Scene::EditorStatusBit::
                                                  EditorPlacement_Always );
                }
...
void SystemDraw()
{
    ...
    if( Scene::IsEdit() )
    {
        if( show_grid )
        {
            ...
            {
                DrawLine3D( VGet( x, 0.0f, -size ),
                            VGet( x, 0.0f, +size ),
                            GetColor( 224, 224, 224 ) );
            }
            for( f32 z = -size; z <= size; z += 1.0f )
            {
                DrawLine3D( VGet( -size, 0.0f, z ),
                            VGet( +size, 0.0f, z ),
                            GetColor( 224, 224, 224 ) );
            }

            // X軸
            DrawLine3D( VGet( -size, 0.0f, 0.0f ),
                        VGet( +size, 0.0f, 0.0f ),
                        GetColor( 255, 64, 64 ) );
            // Y軸
            DrawLine3D( VGet( 0.0f, -size, 0.0f ),
                        VGet( 0.0f, +size, 0.0f ),
                        GetColor( 64, 255, 64 ) );
            // Z軸
            DrawLine3D( VGet( 0.0f, 0.0f, -size ),
                        VGet( 0.0f, 0.0f, +size ),
                        GetColor( 64, 64, 255 ) );
        }
    }
    ...
    // トーンマッピング適用
    CopyToRenderTarget( GetBackBuffer(),
                        texture_hdr_.get(),
