...
namespace
{
    ...
    std::shared_ptr<Texture>
        texture_depth_stencil_;  //!< デフォルトのデプスステンシル
...
bool RenderInit()
{
    ...
    // バックバッファを取得
    //----------------------------------------------------------
    auto* d3d_back_buffer = static_cast<ID3D11Texture2D*>(
        const_cast<void*>( GetUseDirect3D11BackBufferTexture2D() ) );
    auto* d3d_depth_stencil = static_cast<ID3D11Texture2D*>(
        const_cast<void*>( GetUseDirect3D11DepthStencilTexture2D() ) );
...
{
    GetD3DDeviceContext()->ClearDepthStencilView( texture->dsv(),
                                                  D3D11_CLEAR_DEPTH,
                                                  depth_value,
                                                  0 );
}
...
void SetRenderTarget( const TargetDesc& desc )
{
    ...
    {
        rtvs[i] = desc.color_targets_[i] ? desc.color_targets_[i]->rtv()
                                         : nullptr;
    }

    // デプスバッファ
    ID3D11DepthStencilView* dsv = desc.depth_stencil_
                                      ? desc.depth_stencil_->dsv()
                                      : nullptr;
    ...
    // RenderTarget設定
    GetD3DDeviceContext()->OMSetRenderTargets( desc.color_count_,
                                               rtvs.data(),
                                               dsv );
    ...
    //----------------------------------------------------------
    {
        ...
        D3D11_VIEWPORT viewport{
            0, 0, static_cast<f32>( w ), static_cast<f32>( h ), 0.0f, 1.0f
        };
...
//---------------------------------------------------------------------------
void SetRenderTarget( u32       color_count,
                      Texture** color_targets,
...
//---------------------------------------------------------------------------
void CopyToRenderTarget( Texture* dest_render_target,
                         Texture* source_texture,
                         int      shader_ps_handle )
{
    ...
    //----------------------------------------------------------
    {
        ...
        int ps = ( shader_ps_handle != -1 ) ? shader_ps_handle
                                            : *shader_ps_texture_;
